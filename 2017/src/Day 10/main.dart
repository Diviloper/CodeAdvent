...
void first() {
  ...
  for (final length in lengths) {
    final reversed = list.followedBy(list).skip(currentPosition).take(length).toList().reversed.toList();
...
void second() {
  ...
  final denseHash = [
    for (int i=0; i<16; ++i)
      list.sublist(i*16, (i+1)*16).reduce((value, element) => value ^element),
  ];
