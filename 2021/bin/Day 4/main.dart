...
void main() {
  ...
  final boards = <Board>[
    for (final board in input.skip(1)) Board.fromString(board.split('\r\n'))
  ];
...
void findLastBoardToWin(List<int> numbers, List<Board> boards) {
  ...
  for (number in numbers) {
    for (final board in boards) {
      if (!board.hasWon && board.mark(number) && board.hasWon) {
        last = board;
      }
    }
...
class Board {
  ...
  Board.fromString(List<String> rows) {
    ...
    for (final row in rows.map((e) =>
        e.trim().replaceAll('  ', ' ').split(' ').map(int.parse).toList())) {
  ...
  bool get hasWon =>
      rows.any((element) => element.isEmpty) ||
      columns.any((element) => element.isEmpty);
  ...
  int get sum => rows
      .map((row) => row.fold<int>(0, (v, e) => v + e))
      .reduce((v, e) => v + e);
}
